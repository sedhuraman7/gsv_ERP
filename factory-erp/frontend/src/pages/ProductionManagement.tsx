...
const ProductionManagement: React.FC = () => {
    ...
    const stages = [
        ...
        {
            name: 'In Production',
            count: productionOrders.filter(o => o.status === 'in-progress' || o.status === 'in_production').length,
            status: 'ok'
        },
        {
            name: 'Quality Check',
            count: productionOrders.filter(o => o.status === 'qa-check').length,
            status: 'ok'
        },
        {
            name: 'Completed',
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
                {stages.map((stage, index) => (
                    <div key={index} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                        {stage.status === 'delay' && (
                            ...
                            <div key={order.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600">Progress ({Math.round((order.completed / order.quantity) * 100)}%)</span>
                                        <span className="font-medium text-gray-900">{order.completed} / {order.quantity} Units</span>
                                    </div>
